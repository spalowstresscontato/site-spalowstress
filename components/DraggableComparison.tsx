...
import { ZapOff, Heart, ImagePlus, ChevronLeft, ChevronRight } from 'lucide-react';

const STORAGE_BEFORE = 'spa_low_stress_comp_before';
const STORAGE_AFTER = 'spa_low_stress_comp_after';
...
const DraggableComparison: React.FC = () => {
  ...
  useEffect(() => {
    const savedBefore = localStorage.getItem(STORAGE_BEFORE);
    const savedAfter = localStorage.getItem(STORAGE_AFTER);
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>, target: 'before' | 'after') => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (target === 'before') {
          setImgBefore(base64);
          localStorage.setItem(STORAGE_BEFORE, base64);
        } else {
          setImgAfter(base64);
          localStorage.setItem(STORAGE_AFTER, base64);
        }
      };
          ...
          <div className="relative sticky top-24">
            {showAdmin && (
              <div className="absolute -top-12 left-0 right-0 flex justify-center gap-2 z-30 animate-fade-in">
                <button onClick={() => fileBeforeRef.current?.click()} className="bg-white text-gray-700 px-3 py-1.5 rounded-full text-xs font-bold shadow-md border flex items-center gap-1 hover:bg-gray-50">
                  <ImagePlus size={14} /> Foto Estressado
                </button>
                <button onClick={() => fileAfterRef.current?.click()} className="bg-white text-purple-600 px-3 py-1.5 rounded-full text-xs font-bold shadow-md border border-purple-100 flex items-center gap-1 hover:bg-purple-50">
                  <ImagePlus size={14} /> Foto Relaxado
                </button>
            ...
            >
              <div className="relative w-full aspect-[4/5] md:aspect-square rounded-[3rem] overflow-hidden shadow-2xl cursor-ew-resize border-8 border-white">
